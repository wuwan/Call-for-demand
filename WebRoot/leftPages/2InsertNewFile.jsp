...
///////////////////////////////////基础研究，非基础研究////////////////////////////////////
function show(){
	var s=document.getElementsByName('need.CResearch');
...
<form action="needAdd.action"  method=post id="form1" name="form1" onSubmit="return star()">
  <br>
	  ...
	  <!-- --------------------------------------------                 -->
				  <tr>
				    ...
				    <td colspan="6">
				    <input type="text" name="need.CNumber" value="<%out.println(CNumber);%>" class="full" readonly="readonly"></td>
	    </tr>
	    
	    <tr align="center">
								<td width="20"><input name="need.number" value="${user.number}" type="hidden" id="Number" readonly="readonly">
							</tr>
	    <!-- -----------------------------归口管理部门---------------                 -->
	    <tr>
				    <td  width="10%"><span>*</span>机构全称</td>
				    <td width="40%" colspan="2"><input type="text" name="need.name" id="Name" class="full" value="${user.name}" readonly="readonly"></td>
				    <td  width="10%" colspan="2"><span>*</span>归口管理部门</td>
				    <td width="40%" colspan="2"><select name="need.CBelong" id="need.CBelong">  
                      <c:forEach items="${belong}" var="item">        
                      <option>${item.trepresent}</option>  
                      </c:forEach>                             
                      </select> 
		            </td>
				    </tr>
	    ...
	    <!-- --------------------------------------------                 -->
				  <tr>
				    ...
				    <td width="40%" colspan="2"><input type="text" name="need.address" id="Address" class="full" value="${user.address}" readonly="readonly"></td>
				    <td  width="10%" colspan="2"><span>*</span>所在地域</td>
		
				   <td width="40%" colspan="2"><select name="need.CMoneyElse" id="need.CMoneyElse">  
                      <c:forEach items="${area}" var="item">        
                      <option>${item.ARepresent}</option>  
                      </c:forEach>                             
                      </select> 
		            </td>
	    </tr>
	    ...
	    <!-- --------------------------------------------                 -->
				  <tr>
				    ...
				    <td colspan="2" width="40%"><input type="text" name="need.poster" id="Poster" class="full" value="${user.poster}" readonly="readonly"></td>
        </tr>
        <!-- --------------------------------------------                 -->
				  <tr>
	    ...
	    <!-- --------------------------------------------限500字以内             -->
				  ...
				  <tr>
				    <td  colspan="7"><textarea name="need.CIntroduce" id="C_introduce"rows="10" wrap=PHYSICAL cols="130" onKeyDown="gbcount(this.form.C_introduce,this.form.total,this.form.used,this.form.remain);" onKeyUp="gbcount(this.form.C_introduce,this.form.total,this.form.used,this.form.remain);" onblur="check2_1()"></textarea></td>
	    </tr>
	    <tr><td>
	    输入情况
	    </td><td>最多字数： </td>
	    ...
	    <tr>
				  ...
				  <tr>
				    <td  colspan="7"><textarea name="need.CImportant" id="C_important" rows="10" cols="100" maxlength='500' onchange="check30()"></textarea><span id="em1" style="display: inline;"></span></td>
	    </tr>
	    <tr>
				    <td colspan="7" align="center">2、技术关键（所需的关键技术、主要指标，限500字以内）</td>
	    </tr>
	    <tr>
				    <td  colspan="7"><textarea name="need.CMoneyLoad" id="C_money_load" rows="10" cols="100"  maxlength='500'></textarea></td>
	    </tr>
	    <tr>
				    <td colspan="7" align="center">3、预期目标（技术创新性、经济社会效益，限500字以内）</td>
	    </tr>
	    <tr>
				    <td  colspan="7"><textarea name="need.CMoneyTotal" id="C_money_total" rows="10" cols="100"  maxlength='500'></textarea></td>
	    </tr>
	    <!-- --------------------------------------------                 -->
	    <tr>
				    <td colspan="2"><span>*</span>关键字</td>
				    <td >
				    <input type="text" name="need.CKey" id="C_key"class="full" onblur="check13()" onkeydown="k(C_key1)"></td>
				    <td ><input type="text" name="need.CKey1" id="C_key1"class="full" style="display:none" onblur="check17()" onkeydown="k(C_key2)"></td>
				    <td ><input type="text" name="need.CKey2" id="C_key2"class="full" style="display:none" onblur="check18()" onkeydown="k(C_key3)"></td>
				    <td ><input type="text" name="need.CKey3" id="C_key3"class="full" style="display:none" onblur="check19()" onkeydown="k(C_key4)"></td>
				    <td ><input type="text" name="need.CKey4" id="C_key4"class="full" style="display:none" onblur="check20()">
				    </td>                  
	    </tr>
	    <!-- --------------------------------------------                 -->
	    <!-- --------------------------------------------                 -->
     ...
     <!-- --------------------------------------------                 -->
	 <tr>  
	    <td>学科分类<span>*</span></td>
	    <td colspan="6"><select name="fir" id="fir" onchange="getSecondXkfls()">
			<option>-----请选择-----</option>
		</select>
	    ...
	    <select name="thi" id="thi" onchange="inputXkfl()">
      	...
      	<input name="need.CStudy" type="hidden" id="GMJJHY"/>
        </td>
      </tr>
	    <!-- --------------------------------------------                 -->
	    <tr>
...
（非基础研究填写）</td>
	    ...
	    <tr>
	        ...
	        <td colspan="6"><select name="fir_h" id="fir_h" onchange="getSecondGmjjhys()">
		           <option>-----请选择-----</option>
		      </select>
	        ...
	        <select name="thi_h" id="thi_h" onchange="inputGmjjhy()">
      	    ...
      	    <input name="need.CApplication" type="hidden" id="XKFL" />
            </td>
        </tr>
        <!-- --------------------------------------------                 -->
	  ...
	  <input type="submit" name="submit1" value="保存">
				<input type="submit" name="submit2" onclick="return confirm('需求信息审核后不能修改！ 确定提交吗')" value="保存并提交">
  </form>
  </center>
  </body>
</html>
